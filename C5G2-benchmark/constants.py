...
def materials_het():
    # Capilla. Applications of the multidimensional PL equations to complex fuel assembly problems. 2009.
...
def materials_hom():
    ...
    mu1b = np.array([1.013854, 0.996026, 1.013854, 0.996026])

    mu2 = np.array([1.003107, 0.999433])
    mu2b = np.array([1.003107, 0.999433, 1.003107, 0.999433])
...
def create_xs(outdir, temp, materials):
    ...
    for currentMat in materials.keys():
        ...
        with open(outdir + '/' + currentMat +
            ...
            strData = materials[currentMat]['totxs'] - materials[currentMat]['scxs'].reshape((2, 2)).diagonal()
            strData = ' '.join(
            [str(dat) for dat in strData]) if isinstance(
                strData, np.ndarray) else strData
            fh.write(str(temp) + ' ' + strData)
        ...
        with open(outdir + '/' + currentMat +
            ...
            strData = materials[currentMat]['totxs'] + 4./5*(materials[currentMat]['totxs'] - materials[currentMat]['scxs'].reshape((2, 2)).diagonal())
            strData = ' '.join(
            [str(dat) for dat in strData]) if isinstance(
                strData, np.ndarray) else strData
            fh.write(str(temp) + ' ' + strData)
        ...
        with open(outdir + '/' + currentMat +
            ...
            strData = 2*(materials[currentMat]['totxs'] - materials[currentMat]['scxs'].reshape((2, 2)).diagonal())
            strData = ' '.join(
            [str(dat) for dat in strData]) if isinstance(
                strData, np.ndarray) else strData
            fh.write(str(temp) + ' ' + strData)
        ...
        with open(outdir + '/' + currentMat +
            ...
            strData = 2./5*(materials[currentMat]['totxs'] - materials[currentMat]['scxs'].reshape((2, 2)).diagonal())
            strData = ' '.join(
            [str(dat) for dat in strData]) if isinstance(
                strData, np.ndarray) else strData
            fh.write(str(temp) + ' ' + strData)
        ...
        for coeff in ['CHIT', 'CHIP', 'CHID', 'FISS', 'KAPPA', 'INVV']:
            with open(outdir + '/' + currentMat + '_' + coeff + '.txt', 'a') as fh:
